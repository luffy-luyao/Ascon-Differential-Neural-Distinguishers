...
def ascon_hash(plaintext, Nr):
    ...
    iv = int_to_bytes(0x0000080100cc0003,8)
    ...
    '''
    H = np.zeros((plaintext.shape[0],1), dtype=np.uint64)
    for i in range(1):
        H[:,i] ^=A[:,0]
        A = ascon_permutation(A, Nr,t)
    '''
...
def to_binary(A):
    binary_array = np.unpackbits(A.view(np.uint8).reshape(-1, 8)[:, ::-1], axis=1).reshape(-1, 64)
    return binary_array
